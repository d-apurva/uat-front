...
var DashBoard = {
    ...
    RateUatDashboardClick: function () {
        var title = $('#txtTitle').val();
        var feedback = $('#txtFeedback').val();
        var feature = $('#txtFeature').val();
        var drpPriority = $('#drpPriority').val();
        //var val = document.getElementById('<%=(Master.FindControl("userW")).ClientID %>').innerText;
       // alert(document.getElementById('<%=((Label)Master.FindControl("userW")).ClientID %>').innerText);
        var user = $('#userW');
        var ClientID = user[0].outerText;
        var data = {
            "title": title,
            "feedback": feedback,
            "feature": feature,
            "priority": drpPriority,
            "UserNm": ClientID
        }
        var result = ServiceLayer.InsertUpdateData("InsertRateUATDashboard", data, "Dashboard");
        $('#txtTitle').val("");
        $('#txtFeedback').val("");
        $('#txtFeature').val("");
        $('#drpPriority').val('Medium');
        $('#myModal').css('display', 'none');
        DashBoard.alertBox(' saved successfully.');
    },
        ...
        if (projectActivityItems != null && projectActivityItems != undefined) {
            ...
            for (var i = 0; i < projectActivityItems.length; i++) {
                ...
                if (isPortfolioOn) {
                    ...
                    {
                    //by kr
                        str += '<td title="' + testPassName + '"><a target="_blank" href=' + Main.getSiteUrl() + '/TestPass?pid=' + projectID + '&tpid=' + testpassID + '&edit=1' + '>' + trimText(testPassName, 28) + '</a></td>'; //Code modified by Rajiv on 23 feb 2012
